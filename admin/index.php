<?php
$require_once_line = "require_once __DIR__ . '/../auth.php';";
      ...
      <?php
        try {
          global $mysqli;
            switch ($view) {
                case 'users':
              $res = $mysqli->query("SELECT id, name, email, role, created_at FROM users ORDER BY id DESC LIMIT {$limit}");
              $rows = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
                    $cols = ['id','name','email','role','created_at'];
                    break;
                case 'exams':
              $res = $mysqli->query("SELECT id, name, slug, description, created_at FROM exams ORDER BY id DESC LIMIT {$limit}");
              $rows = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
                    $cols = ['id','name','slug','description','created_at'];
                    break;
                case 'test_papers':
              $res = $mysqli->query("SELECT id, exam_id, author_id, title, duration_minutes, total_marks, is_published, created_at FROM test_papers ORDER BY id DESC LIMIT {$limit}");
              $rows = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
                    $cols = ['id','exam_id','author_id','title','duration_minutes','total_marks','is_published','created_at'];
                    break;
                case 'questions':
              $res = $mysqli->query("SELECT id, author_id, question_type, default_marks, created_at FROM questions ORDER BY id DESC LIMIT {$limit}");
              $rows = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
                    $cols = ['id','author_id','question_type','default_marks','created_at'];
                    break;
                case 'options':
              $res = $mysqli->query("SELECT id, question_id, option_label, option_text, is_correct FROM options ORDER BY id DESC LIMIT {$limit}");
              $rows = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
                    $cols = ['id','question_id','option_label','option_text','is_correct'];
                    break;
                case 'test_paper_questions':
              $res = $mysqli->query("SELECT id, test_paper_id, question_id, position, marks FROM test_paper_questions ORDER BY id DESC LIMIT {$limit}");
              $rows = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
                    $cols = ['id','test_paper_id','question_id','position','marks'];
                    break;
                case 'user_attempts':
              $res = $mysqli->query("SELECT id, user_id, test_paper_id, status, score, started_at, finished_at FROM user_attempts ORDER BY id DESC LIMIT {$limit}");
              $rows = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
                    $cols = ['id','user_id','test_paper_id','status','score','started_at','finished_at'];
                    break;
                case 'user_answers':
              $res = $mysqli->query("SELECT id, attempt_id, question_id, selected_option_id, answer_text, is_correct, marks_obtained, answered_at FROM user_answers ORDER BY id DESC LIMIT {$limit}");
              $rows = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
